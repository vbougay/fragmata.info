...
- **PDF-first releases**: sometimes a fresh bulletin appears only as a PDF link (e.g. `20-JULY-2026-GR.pdf`) while both pages still link an older XLSX. The matching XLSX is usually **already uploaded but not yet linked** — probe the predictable URLs directly (HTTP HEAD/GET) before concluding there's no new data:
  ...
  - Only if no XLSX exists anywhere, extract the numbers from the PDF itself (single-page table, same layout as the sheet)
